...
<template>
  <el-button link v-if="roomStore.userUuid === roomStore.room.owner" class="icon-button" :underline="false"
    @click="openDialog">
    <el-icon size="25">
      <Setting />
    </el-icon>
  ...
  <el-dialog v-model="dialogFormVisible" :title="t('settings.form.title')">
    <el-row>
      ...
      <el-col :span="6">
        <select @change="selectList" v-model="currentList">
          ...
          <option v-for="(set, index) in cardsSets" :key="index" :value="set.values.split(';')">{{ t(set.name) }}</option>
        </select>
      </el-col>
    </el-row>
    ...
    <div v-if="currentList.length != 0">
      <el-row class="list">
        <el-col :span="2" v-for="(val,index) in currentList" :key="index">
