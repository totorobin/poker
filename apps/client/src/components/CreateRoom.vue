<script lang="ts" setup>
  ...
  import { type SavedRoom } from '@poker/shared'
      ...
      <div>
        ...
        >
          ...
          <template #default="{ item }">
            <div class="items" @keydown.delete="() => removeSetting(item.id)">
              <span>{{ item.value }}</span>
              <span class="cards">
                <span v-for="card in item.room.cards" :key="card" class="card">{{ card }}</span>
              </span>
            </div>
          </template>
        </el-autocomplete>
